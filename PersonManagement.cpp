...
void display_Siblings(Person* head)
{
    ...
    while(current->next != NULL) // Compare target person's Mother SSN to the SSN of everyone else
    {
        if(targetPerson->motherSSN == current->motherSSN && current->personSSN != targetPerson->personSSN || targetPerson->fatherSSN == current->fatherSSN && current->personSSN != targetPerson->personSSN) //Compare to motherSSN than ensure personSSN does not show in output
        {
